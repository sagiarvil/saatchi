...
import { Watch, ShieldCheck, Truck, Award, ArrowRight, Sparkles } from 'lucide-react';
...
export function FeaturedLuxuryWatches({ allWatches }: Props) {
  ...
  const filteredWatches = useMemo(() => {
    ...
    if (activeTab === "rolex") {
      ...
      // Tüm başyapıtlar için: En yüksek fiyatlı Rolex, Cartier, TAG Heuer ve Rado modellerinden dengeli bir seçki
    ...
    // Seçilen marka için en prestijli ilk 6 veya 8 saati çift sayı olarak al (mobilde 2 kolonda asimetri olmasın)
  ...
  return (
    <section className="w-full bg-[#FAF8F5] py-14 sm:py-20 md:py-28 px-3.5 sm:px-6 lg:px-8 relative overflow-hidden border-b border-[#E8E2D5]">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-96 bg-[radial-gradient(circle_at_top,rgba(194,167,104,0.15),transparent_70%)] pointer-events-none" />

      <div className="w-full max-w-[1536px] mx-auto relative z-10">
        
        {/* Section Header */}
        <div className="text-center mb-8 sm:mb-12 md:mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#846b32]/10 border border-[#846b32]/25 mb-3">
          ...
          {filteredWatches.map((watch) => {
            ...
            return (
              <Link 
                href={watch.seoUrl} 
                key={watch.id} 
                className="group cursor-pointer flex flex-col bg-white rounded-2xl border border-[#E8E2D5] overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_35px_-10px_rgba(132,107,50,0.2)] hover:border-[#846b32]/50 transition-all duration-500 hover:-translate-y-1 relative"
              >
                {/* Watch Presentation Pedestal / Image Stage */}
                <div className="w-full aspect-square sm:aspect-[4/5] bg-gradient-to-b from-[#FCFBF8] via-[#F8F5EE] to-[#EFEAE0] relative flex items-center justify-center overflow-hidden border-b border-[#EFECE3]">
                  ...
                  <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 z-10">
                    <span className="inline-flex items-center gap-1 text-[8px] sm:text-[9px] tracking-wider uppercase font-semibold text-[#846b32] bg-white/90 backdrop-blur-md px-2 py-0.5 rounded-full border border-[#846b32]/20 shadow-xs">
                  ...
                  <div className="absolute inset-0 w-full h-full flex items-center justify-center p-3 sm:p-5 md:p-6 z-0">
                    {watch.image ? (
                      <Image 
                        ...
                        className="object-contain p-2 sm:p-3 group-hover:scale-110 transition-transform duration-700 ease-out mix-blend-multiply drop-shadow-[0_8px_16px_rgba(0,0,0,0.1)]" 
                      />
                    ) : (
                  ...
                  <div className="absolute bottom-2 right-2 sm:bottom-3 sm:right-3 z-10 opacity-90 group-hover:opacity-100 transition-opacity">
                    <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/90 backdrop-blur-md border border-[#846b32]/30 flex items-center justify-center text-[#846b32] group-hover:bg-[#846b32] group-hover:text-white transition-all shadow-xs">
                ...
                <div className="p-3 sm:p-4 md:p-5 flex flex-col flex-grow justify-between text-left">
                  <div>
                    {/* Brand name */}
                    <div className="flex items-center justify-between gap-1 mb-1">
                      ...
                      {refCode && (
                        <span className="text-[8px] sm:text-[9px] tracking-wider text-[#9E9585] font-mono">
                    ...
                    {/* Model Name */}
                    <h4 
                      className="text-xs sm:text-sm md:text-[15px] font-serif text-[#1F2421] mb-2 sm:mb-3 group-hover:text-[#846b32] transition-colors line-clamp-2 leading-snug font-medium"
                  ...
                  {/* Price and CTA */}
                  <div className="pt-2 sm:pt-3 border-t border-[#F2EFE8] flex items-baseline justify-between">
                    <div>
                      <span className="text-[8px] sm:text-[9px] text-[#8C8474] tracking-wider uppercase block font-medium">
                        Saatchi Fiyatı
                      </span>
                      <p className="text-sm sm:text-base md:text-lg font-semibold text-[#1F2421] tracking-tight">
                        {watch.price}
                      </p>
                    </div>

                    <span className="hidden sm:inline-block text-[10px] tracking-widest text-[#846b32] font-semibold uppercase group-hover:underline">
                      İncele
                    </span>
                  </div>
                </div>
        ...
        <div className="mt-12 sm:mt-16 pt-8 sm:pt-12 border-t border-[#E8E2D5]/80 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 text-center">
          <div className="flex items-center sm:flex-col sm:justify-center gap-3 p-3.5 rounded-xl bg-white/70 border border-[#E8E2D5] shadow-xs">
