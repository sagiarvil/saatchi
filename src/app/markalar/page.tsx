    ...
    <div className="min-h-screen bg-[#0d0c0b] text-[#f5f0e8]">
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 px-5 py-16 sm:px-8 md:py-20 lg:grid-cols-[1fr_420px] lg:items-end lg:px-12">
          ...
          <div className="border-l border-[#7f262b] pl-6"><p className="text-sm leading-7 text-[#aaa29a]">Rolex, Cartier, TAG Heuer ve Rado Elit Kategori altında; Tissot, Carren, Calvin Klein, Michael Kors ve Versace Saat kategorisinde konumlanır. Katalog fiyatları kaynak veriye göre güncellenir.</p><Link href="/elit-saat" className="mt-6 inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#e4d8cb]">Elit koleksiyona geç <ArrowUpRight className="h-4 w-4" /></Link></div>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-7xl px-5 py-12 sm:px-8 md:py-16 lg:px-12">
        ...
        <section className="mt-14 grid gap-6 border-t border-white/10 pt-10 md:grid-cols-[1fr_auto] md:items-center"><div><p className="text-[10px] font-semibold uppercase tracking-[0.26em] text-[#8e6668]">Fiyat politikası</p><h2 className="mt-3 text-2xl font-medium tracking-[-0.025em] sm:text-3xl">Kaynak fiyat + %50. Üst sınır 1.799.000 TL.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#8d857e]">Kaynakta fiyatı doğrulanamayan ürün yayınlanmaz. 1.799.000 TL üzerindeki modeller katalog dışında tutulur.</p></div><Link href="/iletisim" className="inline-flex items-center justify-center gap-2 border border-white/15 px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.19em] text-[#e7dfd6] transition-colors hover:border-[#7f262b] hover:bg-[#7f262b]/10">Concierge ile görüş <ArrowUpRight className="h-4 w-4" /></Link></section>
      </main>
    </div>
  );
}
