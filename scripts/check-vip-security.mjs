...
const requirements = [
  ...
  ['production deploy enables framework-aware Firebase Hosting', productionRelease.includes('firebase experiments:enable webframeworks')],
  ['production deploy binds actAs only on the framework runtime service account', productionRelease.includes('390979735373-compute@developer.gserviceaccount.com') && productionRelease.includes('roles/iam.serviceAccountUser') && productionRelease.includes('RUNTIME_SERVICE_ACCOUNT_ACTAS_VERIFIED') && !productionRelease.includes('gcloud projects add-iam-policy-binding')],
